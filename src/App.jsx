...
function Btn({ children, onClick, disabled, full, secondary, small }) {
  ...
  const base = { fontFamily: S.font, fontWeight: 500, fontSize: small ? 14 : 16, cursor: disabled ? "not-allowed" : "pointer", borderRadius: S.radius, padding: small ? "8px 18px" : "12px 32px", width: full ? "100%" : "auto", transition: "background 0.15s, border-color 0.15s", border: "none" };
...
function PromptCard({ prompt, pack }) {
  return (
    <div style={{ background: S.bgGray, border: `1px solid ${S.borderLight}`, borderLeft: `4px solid ${pack.color}`, borderRadius: S.radiusMd, padding: "20px 20px", marginBottom: "1.5rem" }}>
...
export default function App() {
  const [screen, setScreen] = useState("name"); // name | lobby | wait | answer | reveal | end
  ...
  useEffect(() => {
    ...
    const poll = async () => {
      try {
        const r = await window.storage.get(roomKey(roomCode));
        if (!r) return;
        ...
        if ((screen === "reveal" || screen === "answer") && data.phase === "end") setScreen("end");
        if (screen === "lobby" && data.phase === "pack") setRoom(data);
      } catch(e) {}
    };
  ...
  const saveRoom = async (data) => {
    await window.storage.set(roomKey(data.code), JSON.stringify(data), true);
    setRoom(data);
  };
  ...
  const createRoom = async () => {
    ...
    const data = {
      code, phase: "pack", hostName: myName,
      players: [myName], round: 1, packId: null,
      prompts: [], answers: {}, reactions: {},
    };
    await saveRoom(data);
    setRoomCode(code);
    setIsHost(true);
  ...
  const joinRoom = async () => {
    ...
    if (!code) return;
    try {
      const r = await window.storage.get(roomKey(code));
      if (!r) { setErr("Room not found. Check the code and try again."); return; }
      const data = JSON.parse(r.value);
    ...
    } catch(e) { setErr("Could not join room. Try again."); }
  };

  // --- HOST: select pack & start ---
  ...
  const addReaction = async (player) => {
    const data = { ...room };
    data.reactions = { ...data.reactions, [player]: (data.reactions[player] || 0) + 1 };
    await saveRoom(data);
  ...
  // SCREENS
  return (
    <div style={{ fontFamily: S.font, background: S.bg, minHeight: 560, maxWidth: 480, margin: "0 auto", padding: "0" }}>
      ...
      {screen === "lobby" && !roomCode && (
        <div style={{ padding: "2.5rem 1.5rem" }}>
          ...
          <div style={{ background: S.bgGray, borderRadius: S.radiusMd, padding: "20px", marginBottom: "1rem", border: `1px solid ${S.borderLight}` }}>
            <p style={{ margin: "0 0 6px", fontWeight: 600, fontSize: 15, color: S.text }}>Start a new game</p>
          ...
          <div style={{ background: S.bgGray, borderRadius: S.radiusMd, padding: "20px", border: `1px solid ${S.borderLight}` }}>
            <p style={{ margin: "0 0 6px", fontWeight: 600, fontSize: 15, color: S.text }}>Join a room</p>
            <p style={{ margin: "0 0 10px", fontSize: 13, color: S.textSecondary }}>Enter the room code from your host</p>
            <input
              value={joinInput}
              onChange={e => { setJoinInput(e.target.value.toUpperCase()); setErr(""); }}
              onKeyDown={e => e.key === "Enter" && joinRoom()}
              placeholder="Room code e.g. XK9TW"
      ...
      {/* ── HOST LOBBY: waiting for players + pack select ── */}
      {screen === "lobby" && roomCode && isHost && room && (
        <div style={{ padding: "2.5rem 1.5rem" }}>
          <div style={{ background: S.blueLite, border: `1px solid ${S.blueLight2}`, borderRadius: S.radiusMd, padding: "16px 18px", marginBottom: "1.5rem", display: "flex", justifyContent: "space-between", alignItems: "center" }}>
            <div>
              <p style={{ margin: "0 0 2px", fontSize: 12, fontWeight: 600, color: S.blue, textTransform: "uppercase", letterSpacing: "0.6px" }}>Room code</p>
              <p style={{ margin: 0, fontSize: 28, fontWeight: 700, color: S.blue, letterSpacing: "4px" }}>{roomCode}</p>
            </div>
      ...
      {screen === "wait" && (
        <div style={{ padding: "3rem 1.5rem", textAlign: "center" }}>
          ...
          <p style={{ color: S.textSecondary, fontSize: 15, margin: "0 0 2rem" }}>Waiting for everyone else to answer…</p>
          {room && (
            <div style={{ display: "flex", justifyContent: "center", flexWrap: "wrap", gap: 8 }}>
              {room.players.map((p, i) => {
                const answered = room.answers && room.answers[p];
                return (
                  <div key={p} style={{ display: "flex", flexDirection: "column", alignItems: "center", gap: 6 }}>
                    <div style={{ position: "relative" }}>
                      <Avatar name={p} idx={i} size={40} />
      ...
      {screen === "reveal" && room && pack && (
        <div style={{ padding: "2rem 1.5rem" }}>
          ...
          {isHost ? (
            ...
            <div style={{ textAlign: "center", padding: "12px", background: S.bgGray, borderRadius: S.radiusMd, border: `1px solid ${S.borderLight}`, fontSize: 14, color: S.textSecondary }}>Waiting for the host to continue…</div>
          )}
        </div>
      )}
      ...
      {screen === "end" && room && (
        <div style={{ padding: "2.5rem 1.5rem", textAlign: "center" }}>
          ...
          <Btn onClick={() => { setScreen("name"); setRoomCode(""); setMyName(""); setNameInput(""); setRoom(null); setIsHost(false); }} full>Play again 🎨</Btn>
        </div>
      )}
    </div>
  );
